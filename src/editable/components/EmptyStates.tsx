...
export function EmptyState({
  ...
  return (
    <section className={cn('border border-dashed border-current/20 bg-[#f7f7f5] p-10 text-center', className)}>
      <div className="mx-auto flex h-14 w-14 items-center justify-center bg-current/10">
        <SearchX className="h-6 w-6" />
      </div>
      <h2 className="mt-5 font-serif text-3xl font-normal">{title}</h2>
      <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-current/65">{description}</p>
      <Link href={actionHref} className="mt-6 inline-flex items-center gap-2 bg-[#211e19] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#ef4f2b]">
        {actionLabel}
